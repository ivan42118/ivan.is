...
_gsScope._gsDefine(
  ...
  function () {
    ...
    p.fromTo = function (target, duration, fromVars, toVars, position) {
      ...
      return duration
        ? this.add(Engine.fromTo(target, duration, fromVars, toVars), position)
    ...
    p.set = function (target, vars, position) {
      return this.add(
        new TweenLite(
          target,
          0,
          _defaultImmediateRender(this, vars, null, true)
        ),
        position
      );
    };
    ...
    TimelineLite.exportRoot = function (vars, ignoreDelayedCalls) {
      ...
      while (tween) {
        ...
        if (
          !ignoreDelayedCalls ||
          !(
            tween instanceof TweenLite && tween.target === tween.vars.onComplete
          )
        ) {
    ...
    p.add = function (value, position, align, stagger) {
      ...
      if (value._time || (!value._duration && value._initted)) {
        ...
        if (
          !value._duration ||
          Math.abs(Math.max(0, Math.min(value.totalDuration(), curTime))) -
            value._totalTime >
            0.00001
        ) {
        ...
        if (i === -1) {
          if (this._labels[timeOrLabel] == null) {
            return appendIfAbsent
              ? (this._labels[timeOrLabel] = clippedDuration + offsetOrLabel)
              : offsetOrLabel;
          }
    ...
    p.render = function (time, suppressEvents, force) {
      ...
      if (self._hasPause && !self._forcingPlayhead && !suppressEvents) {
        ...
        if (pauseTween) {
          ...
          pauseTime =
            self._startTime +
            (self._reversed ? self._duration - time : time) / self._timeScale;
        }
      }
      if (time >= totalDur - _tinyNum && time >= 0) {
        ...
        if (
          ...
          (self._duration === 0 &&
            self._rawPrevTime !== _tinyNum &&
            (self._rawPrevTime > 0 || (time < 0 && self._rawPrevTime >= 0)))
        ) {
          callback = "onReverseComplete";
